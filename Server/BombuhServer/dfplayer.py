...
class Player():
    def __init__(self, uart=None, busy_pin=None, config=True, volume=0.5):
        self._volume = None
        if uart is None:
            self.uart = UART(1, 9600, tx=0, rx=1) # UART on 
    ...
    def loop_folder(self, folderNum):
        self.awaitconfig()
        self.command(0x17, 0, folderNum)
    ...
    def resume(self):
        self.awaitconfig()
        self.command(0x0D, 0, 0)

    def eq(self, mode):
        self.awaitconfig()
        self.command(0x7, 0, mode)
    ...
    def volume(self, volume=None):
        ...
        if volume is None:
            ...
            oldval = int(30.0 * self._volume) if self._volume is not None else None
